...
static inline void tape_get_density_code(struct device_data *dev)
{
	unsigned char buf[TC_MP_SUPPORTEDPAGE_SIZE]={0};
	int ret;

	ret = dev->backend->modesense(dev->backend_data, TC_MP_SUPPORTEDPAGE, TC_MP_PC_CURRENT, 0x00, buf, sizeof(buf));
	if (ret < 0) {
		dev->density_code = 0;
		return;
	}
	dev->density_code = buf[8];
