...
extern int iokitosx_takedump_drive(void * device, bool nonforced_dump);
extern void iokitosx_process_errors(void *device, int rc, char *msg, char *cmd, bool take_dump);

static inline void iokitosx_get_dump(void *device, bool nonforced_dump)
{
    //int fd = *((int *)device);

#ifdef __iokit_tc_c
	iokitosx_takedump_drive(device, nonforced_dump);
#endif
