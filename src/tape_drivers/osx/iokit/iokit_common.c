...
int iokitosx_takedump_drive(void *device, bool nonforced_dump)
{
...
static bool is_dump_required_error(void *device, int ret, bool *nonforced_dump)
{
	bool ans = false;
	int err = -ret;
	iokit_device_t *iokit_device = (iokit_device_t *)device;
...
void iokitosx_process_errors(void *device, int rc, char *msg, char *cmd, bool take_dump)
{
	...
	if (device) {
		if (take_dump
			...
			(void)iokitosx_takedump_drive(device, nonforced_dump);
		}
	}
}
