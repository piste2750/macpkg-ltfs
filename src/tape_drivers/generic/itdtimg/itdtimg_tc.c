...
int itdtimage_open(const char *name, void **handle)
{
	...
	*handle = NULL;
	...
	if ( !state->img_file ) {
		ltfsmsg(LTFS_ERR, "12459E", name, " file open fails");
	...
	if (_seek_file(state->img_file, length-read_length)!=0){
		ltfsmsg(LTFS_ERR, "12458E" , state->filename,(long long)length-read_length);
		_itdtimage_free(state);
	...
	if ( ! state->rll_count ){
		ltfsmsg(LTFS_ERR, "12459E", state->filename,"Meta Info [rll_count] is not valid");
		_itdtimage_free(state);
		free(buffer);
		return -EDEV_DEVICE_UNOPENABLE;
	}
	if ( state->version < 2 ){
		ltfsmsg(LTFS_ERR, "12459E", state->filename,"Unsupported ITDT Image file version");
		_itdtimage_free(state);
		free(buffer);
		return -EDEV_DEVICE_UNOPENABLE;
	}
	if ( ! state->byte_count ){
		ltfsmsg(LTFS_ERR, "12459E", state->filename,"Meta Info [byte_count] is not valid");
		_itdtimage_free(state);
		free(buffer);
		return -EDEV_DEVICE_UNOPENABLE;
	}
	...
	if (state->attr_count==0){
		ltfsmsg(LTFS_ERR, "12459E", state->filename,"Meta Info [attr_] is not valid");
		_itdtimage_free(state);
		free(buffer);
		return -EDEV_DEVICE_UNOPENABLE;
	}
	...
	if ( _seek_file(state->img_file, state->byte_count) ) {
		ltfsmsg(LTFS_ERR, "12458E" , state->filename,(long long)state->byte_count);
		_itdtimage_free(state);
		free(buffer);
		return -EDEV_HARDWARE_ERROR;
	}
				   ...
				   const bool unusual_size)
{
	...
	if ( _seek_file(state->img_file, offset) ){
	 	ltfsmsg(LTFS_ERR, "12458E" , state->filename,(long long)offset);
		return -EDEV_HARDWARE_ERROR;
	}
...
{
	struct itdtimage_data *state = (struct itdtimage_data *)vstate;
	ltfsmsg(LTFS_DEBUG, "12450D" , __FUNCTION__ );
	if (!state->ready) {
...
int itdtimage_space(void *vstate, size_t count, TC_SPACE_TYPE type, struct tc_position *pos)
{
	...
	state->current_position.filemarks = count_fm;
	pos->filemarks = state->current_position.filemarks;
	ltfsmsg(LTFS_DEBUG, "12456D" , state->current_position.partition,state->current_position.block,state->current_position.filemarks,(int)state->device_reserved,(int)state->medium_locked,(int)state->ready);
							 ...
							 , unsigned char *buf, const size_t size)
{
	...
	// ltfsmsg(LTFS_DEBUG, "12152D", "readattr", (unsigned long long)part, (unsigned long long) id);
	ltfsmsg(LTFS_DEBUG, "12452D" , part , id );
...
int itdtimage_report_density(void *device, struct tc_density_report *rep, bool medium)
{
	/* Always return LTO5 */
	rep->size = 1;
...
int itdtimage_get_parameters(void *vstate, struct tc_drive_param *drive_param)
{
	drive_param->max_blksize = FILE_DEBUG_MAX_BLOCK_SIZE;
	drive_param->logical_write_protect = true;
	drive_param->write_protect = true;
	return DEVICE_GOOD;
}
...
int itdtimage_takedump_drive(void *device)
{
	/* Do nothing */
	return DEVICE_GOOD;
}

bool itdtimage_is_mountable(void *device, const char *barcode, const unsigned char density_code)
{
	/* Do nothing */
	return true;
}
...
itdtimage_get_serialnumber(void *vstate, char **result)
{
	struct itdtimage_data *state = (struct itdtimage_data *)vstate;
...
struct tape_ops itdtimage_handler = {
	...
	.report_density			= itdtimage_report_density,
...
extern char driver_generic_itdtimg_dat[];
#endif

const char *tape_dev_get_message_bundle_name(void **message_data)
{
#ifndef mingw_PLATFORM
	*message_data = driver_generic_itdtimg_dat;

#else
	*message_data = NULL;
#endif
	return "driver_generic_itdtimg";
}
