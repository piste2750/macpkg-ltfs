...
extern int ibmtape_readbuffer(void *device, int id, unsigned char *buf, size_t offset, size_t len,
							  int type);
extern int ibmtape_takedump_drive(void *device, bool nonforced_dump);
extern int ibmtape_get_serialnumber(void *device, char **result);
 ...
 * @return none
 */
static inline void ibmtape_get_dump(void *device, bool nonforced_dump)
{
	ibmtape_takedump_drive(device, nonforced_dump);
	return;
}
...
static inline void ibmtape_process_errors(void *device, int rc, char *msg, char *cmd, bool take_dump)
{
	...
	if (device) {
		...
		if ( priv->type == DEVICE_TAPE &&
			...
			ibmtape_get_dump(device, nonforced_dump);
	}
}
