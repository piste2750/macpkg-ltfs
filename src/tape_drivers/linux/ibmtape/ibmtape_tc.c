...
static inline int _handle_block_allocation_failure(void *device, struct tc_position *pos, int *retry)
{
    ...
    ltfsmsg(LTFS_WARN, "12228W", ++(*retry));
    sleep(3);

    ret = ibmtape_readpos(device, &tmp_pos);
    if (ret == DEVICE_GOOD && pos->partition == tmp_pos.partition) {
        if (pos->block == tmp_pos.block) {
            /* No record was written on the tape */
            ltfsmsg(LTFS_INFO, "12229I", (unsigned long) tmp_pos.partition, (unsigned long long)tmp_pos.block);
            ret = WRITE_RETRY;
        } else if (pos->block == tmp_pos.block - 1) {
            /* The record was written on the tape */
            ltfsmsg(LTFS_INFO, "12232I",
            ...
            /* Unexpected position */
            ltfsmsg(LTFS_WARN, "12231W", ret,
                    (unsigned long)pos->partition, (unsigned long long)pos->block,
                    (unsigned long)tmp_pos.partition, (unsigned long long)tmp_pos.block);
            ret = -EDEV_NO_MEMORY;
        }
    } else
        ltfsmsg(LTFS_WARN, "12230W", ret,
                (unsigned long)pos->partition, (unsigned long long)pos->block,
                (unsigned long)tmp_pos.partition, (unsigned long long)tmp_pos.block);
...
write_start:
	...
	if ((size_t)written != datacount || errno == ENOSPC) {
		ltfsmsg(LTFS_INFO, "12196I", "WRITE", count, written, errno, ((struct ibmtape_data *) device)->drive_serial);
...
int ibmtape_erase(void *device, struct tc_position *pos, bool long_erase)
{
	...
	if (long_erase) {
		...
		if (rc == -EDEV_TIME_STAMP_CHANGED) {
			ltfsmsg(LTFS_DEBUG, "12233D", "erase", -1, rc);
			rc = ibmtape_long_erase(device);
		}
...
struct tape_ops ibmtape_drive_handler = {
	...
	.get_serialnumber       = ibmtape_get_serialnumber,
};
