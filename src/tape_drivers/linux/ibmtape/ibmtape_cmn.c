...
struct supported_device *supported_devices[] = {
		...
		DEVICE( "ULTRIUM-TD7     ", IBM_3580, DRIVE_LTO7,    " [ULTRIUM-TD7] "), /* IBM Ultrium Gen 7 */
		DEVICE( "ULT3580-TD7     ", IBM_3580, DRIVE_LTO7,    " [ULT3580-TD7] "), /* IBM Ultrium Gen 7 */
		DEVICE( "ULTRIUM-HH7     ", IBM_3580, DRIVE_LTO7_HH, " [ULTRIUM-HH7] "), /* IBM Ultrium Gen 7 Half-High */
		DEVICE( "ULT3580-HH7     ", IBM_3580, DRIVE_LTO7_HH, " [ULT3580-HH7] "), /* IBM Ultrium Gen 7 Half-High */
		DEVICE( "HH LTO Gen 7    ", IBM_3580, DRIVE_LTO7_HH, " [HH LTO Gen 7]"), /* IBM Ultrium Gen 7 Half-High */
...
start:
	...
	if (rc != 0) {
		...
		if (rc == -EDEV_TIME_STAMP_CHANGED) {
			ltfsmsg(LTFS_DEBUG, "12233D", cmd_name, cmd, rc);
			goto start;
		}
 ...
 * @return 0 on success or negative value on error
 */
int ibmtape_takedump_drive(void *device, bool nonforced_dump)
{
 ...
 * @return 0 on success or a negative value on error.
 */
int ibmtape_get_serialnumber(void *device, char **result)
{
	...
	ltfs_profiler_add_entry(bend_profiler, &bend_profiler_lock, CHANGER_REQ_ENTER(REQ_TC_GETSER));

	*result = strdup((const char *) priv->drive_serial);
	if (! *result) {
		ltfsmsg(LTFS_ERR, "10001E", "ibmtape_get_serialnumber: result");
		ltfs_profiler_add_entry(bend_profiler, &bend_profiler_lock, CHANGER_REQ_EXIT(REQ_TC_GETSER));
		return -EDEV_NO_MEMORY;
	}
